...
SELECT DISTINCT(o.oid)                         AS oid,
               ...
               CASE
                   ...
                   END                         AS parent_type,
               CASE
                   WHEN l_from.name IS NULL THEN NULL
                   ELSE CONCAT(l_from_n.prefix, ':', l_from.name)
                   END                         AS link_from,
               l.name                          AS link_name,
               CASE
                   WHEN l_to.name IS NULL THEN NULL
               ...
               CASE
                   WHEN t2.name IS NOT NULL THEN substring(t2.description from 0 for max_description)
...
SELECT *
FROM tlm__report_schema(max_description => 20);
